...
def obter_jogos_gratis():
    params = {"locale": "pt-BR", "country": "BR", "allowCountries": "BR"}
    headers = {"User-Agent": "Mozilla/5.0"}

    response = requests.get(EPIC_API_URL, params=params, headers=headers)
...
def enviar_webhook(jogo):
    payload = {
        "embeds": [
            {
                "title": f"🎮 Jogo Grátis na Epic Games: {jogo['titulo']}",
