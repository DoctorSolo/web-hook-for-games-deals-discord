...
class SteamDeals:
    ...
    def is_mature_or_adult(self, app_id):
        ...
        try:
            ...
            if any(desc_id in self.ADULT_CONTENT_DESCRIPTOR_IDS for desc_id in descriptors):
                return True
    ...
    def is_adult_content_fallback(self, deal):
    ...
    def send_to_discord(self, data):
        ...
        if self.WEBHOOK_URL:
            for embed_group in divide_embeds:
                requests.post(self.WEBHOOK_URL, json={"embeds": embed_group}, timeout=10)
